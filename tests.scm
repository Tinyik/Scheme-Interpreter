...
((lambda (x) (display x)
    (newline) x)
        (quote TreeCutting))
; expect treecutting; treecutting
...
(define (switch color)
  (cond ((eq? color 'Blue) 'Go)
        ((eq? color 'Yellow) 'Bears)
        ((eq? color 'Red) 'IHY)
        ((eq? color 'Error) (/ 1 0))
  )
)
...
((lambda (dup dup)
    (+ x x x x)) 'nil 'cond)
; expect Error: duplicate symbol:

((lambda (I am)
    ((lambda (taking 61A B C together)
        (* (+ taking B (/ 61A I am)) C )) 20 11 12 10 11
    ))
    8 10
)
; expect 321.375
...
(define lam (lambda (da lam) (quotient lam da)))
; expect lam

lam
; expect (lambda (da lam) (quotient lam da))
...
(begin 89 True
    lam
    myfunction)
; expect (lambda (x y) (+ x (* x (quotient x y))))
...
(define tricky (lambda (cookie dynamic)
        (Im (+ cookie cookie))))
...
(and (null? 61C) 80 0 or myfunction)
; expect False
