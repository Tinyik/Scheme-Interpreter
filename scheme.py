...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        args = Pair(expressions.first.second, expressions.second)
        instance = do_lambda_form(args, env)
        env.define(target.first, instance)
        return target.first
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    return LambdaProcedure(formals, second, env)
    # END PROBLEM 9
